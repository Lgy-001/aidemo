...
const currentRef=useRef<HTMLDivElement>(null)
  ...
  const setMessage=useMessageStore((state)=>state.setMessage)
    ...
    return(
        <div className="h-auto max-h-[80vh] min-w-[832px] px-[10px] py-[20px] flex flex-col overflow-y-auto" ref={currentRef}>
            {
                messages.map((items,index)=>{
                    return(
                        <MessageBox  message={items.message} role={items.role} isLoading={messages.length-1===index} key={index} ></MessageBox>
                    )
                })
            }
